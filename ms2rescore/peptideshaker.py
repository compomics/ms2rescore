...
class ExtendedPsmReportAccessor:
    ...
    def __init__(self, pandas_obj: pd.DataFrame) -> None:
        ...
        self._get_mass()
        self._get_len()
        self._get_misscleavages()
        self._set_mass_error_unit()
        self._validate()
    
    def _get_charge(self):
        self._obj['Charge'] = self._obj['Measured Charge'].apply(lambda x: int(x.replace('+','')))

    def _get_mass(self):
        self._obj['Mass'] = self._obj['m/z'] * self._obj['Charge']

    def _get_len(self):
        self._obj['Length'] = self._obj['Sequence'].apply(lambda x: len(x))

    def _get_misscleavages(self):
        self._obj['Missed cleavages'] = self._obj['Sequence'].apply(lambda x: x[:-1].count('K') + x[:-1].count('R'))
