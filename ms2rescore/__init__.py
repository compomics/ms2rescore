...
def main():
    ...
    rescore_core.make_ms2pip_config(config)
    ...
    sys.stdout.flush()
    rescore_core.calculate_features(fname + "_" + config["ms2pip"]["frag"] + "_pred_and_emp.csv", fname + "_ms2pipfeatures.csv", config["ms2pip"]["num_cpu"])
    sys.stdout.write("Done! \n")
    sys.stdout.flush()

    sys.stdout.write("Generating pin file(s)... ")
    sys.stdout.flush()
    rescore_core.write_pin_files(fname + "_ms2pipfeatures.csv", args.peprec, fname)
    sys.stdout.write("Done! \n")
    sys.stdout.flush()
